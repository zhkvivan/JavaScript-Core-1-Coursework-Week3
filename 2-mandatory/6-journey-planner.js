...
function checkCodeIsThere(stringText) {
  let magicWord = "code";
  //edit code below
  stringTextBoolean = stringText.includes(magicWord)
  if (stringTextBoolean) {
...
*/
function isAccessibleByTransportMode(array, transport) {
	return array.includes(transport);
}
...
function journeyPlanner(locations, transportMode) {
	function a(item) {
		return isAccessibleByTransportMode(item, transportMode);
	}
	let newArray = locations.filter(a);
	function b(item) {
		return getLocationName(item);
	}
	return newArray.map(b);
}
