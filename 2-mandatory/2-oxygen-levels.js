...
function findSafeOxygenLevel(arrayOfPlanets) {
	arrayOfPlanets = arrayOfPlanets.filter(
		(item) => item[item.length - 1] === '%'
	);
	arrayOfPlanets = arrayOfPlanets.map((planet) => planet.replace('%', ''));
	let goodPlanet = arrayOfPlanets.find(
		(goodPlanet) => goodPlanet > 19.5 && goodPlanet < 23.5
	);
	if (typeof goodPlanet !== 'undefined') {
		return goodPlanet + '%';
	} else return goodPlanet;
}
/* ======= TESTS - DO NOT MODIFY ===== */
